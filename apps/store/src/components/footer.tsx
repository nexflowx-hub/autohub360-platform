...
export function Footer({ market: marketCode = 'BR' }: { market?: MarketCode }) {
  ...
  return <footer className="mt-auto bg-navy-950 text-slate-300">
    ...
    <Container className="grid gap-10 py-12 md:grid-cols-2 lg:grid-cols-5">
      <div className="lg:col-span-2"><LogoHorizontal size="md"/><p className="mt-4 max-w-sm text-sm leading-relaxed text-slate-400">Tecnologia para o carro, para a casa e para o seu dia. Peças, acessórios, eletrônicos, segurança e soluções inteligentes com uma operação preparada para Brasil e Europa.</p><div className="mt-5 flex items-center gap-3">{SOCIAL.instagram?<a href={SOCIAL.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram AutoHub360" className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 hover:bg-ahblue-500"><Instagram className="h-4.5 w-4.5"/></a>:null}{SOCIAL.facebook?<a href={SOCIAL.facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook AutoHub360" className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 hover:bg-ahblue-500"><Facebook className="h-4.5 w-4.5"/></a>:null}{SOCIAL.youtube?<a href={SOCIAL.youtube} target="_blank" rel="noopener noreferrer" aria-label="YouTube AutoHub360" className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 hover:bg-ahblue-500"><Youtube className="h-4.5 w-4.5"/></a>:null}<a href={whatsappLink('support')} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp AutoHub360" className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 hover:bg-[#25d366]"><MessageCircle className="h-4.5 w-4.5"/></a></div><div className="mt-6"><PaymentMethods config={payCfg} market={marketCode}/></div></div>
      ...
      <nav aria-label="Ajuda e suporte"><h3 className="mb-4 font-display text-sm font-bold uppercase tracking-wider text-white">Ajuda e suporte</h3><ul className="space-y-2.5 text-sm">{[
        ['/rastrear','Rastrear pedido'],['/pos-venda','Pós-venda e assistência'],['/atendimento','Atendimento'],['/instalacao','Instalação em Anápolis'],['/kits','Kits e combos'],['/conta/pedidos','Meus pedidos'],['/legal/trocas-devolucoes-arrependimento','Trocas e devoluções'],['/legal/garantia','Garantia'],['/legal/entrega-e-frete','Entrega e frete'],['/legal/pagamentos-e-seguranca','Pagamentos e segurança'],['/canal-de-reclamacoes','Canal de reclamações']
      ].map(([href,label])=><li key={href}><Link href={href} className="hover:text-white">{label}</Link></li>)}</ul></nav>
      <nav aria-label="Institucional"><h3 className="mb-4 font-display text-sm font-bold uppercase tracking-wider text-white">Institucional</h3><ul className="space-y-2.5 text-sm">{[
        ['https://autohub360.tech/sobre','Sobre a AutoHub360'],['https://autohub360.tech/pro','AutoHub360 Pro (B2B)'],['https://autohub360.tech/hub','Blog / Hub'],['/legal/termos-de-uso','Termos de uso'],['/legal/termos-e-condicoes-de-venda','Termos de venda'],['/legal/privacidade-lgpd','Privacidade e LGPD'],['/legal/politica-de-cookies','Cookies'],['/legal/termos-de-instalacao','Termos de instalação'],['/legal/acessibilidade-contato','Acessibilidade']
      ].map(([href,label])=><li key={href}><Link href={href} className="hover:text-white">{label}</Link></li>)}</ul></nav>
    </Container>
